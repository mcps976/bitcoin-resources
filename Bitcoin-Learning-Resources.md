# Bitcoin Learning Resources

A comprehensive, curated collection of Bitcoin learning materials, development resources, and community tools. From beginner guides to advanced protocol development.

> **Note:** This is a living document. Contributions and suggestions are welcome!

---

## Table of Contents

- [Essential Reading](#essential-reading)
- [Official Documentation](#official-documentation)
- [Video Resources](#video-resources)
- [Development Resources](#development-resources)
- [Explorers & Tools](#explorers--tools)
- [News & Updates](#news--updates)
- [Security & Privacy](#security--privacy)
- [Lightning Network](#lightning-network)
- [Community Resources](#community-resources)
- [Academic Resources](#academic-resources)
- [Advanced Topics](#advanced-topics)
- [Wallets](#wallets)
- [Organizations](#organizations)
- [Data & Statistics](#data--statistics)

---

## 📖 Essential Reading

### Books

#### **The Bitcoin Standard** - Saifedean Ammous
Economics and monetary history perspective on Bitcoin.
- Website: https://saifedean.com/the-book

#### **Mastering Bitcoin (3rd Edition)** - Andreas Antonopoulos & David Harding
Technical deep dive into Bitcoin's inner workings.
- GitHub: https://github.com/bitcoinbook/bitcoinbook
- Free Online: https://www.oreilly.com/library/view/mastering-bitcoin-3rd/9781098150082/

#### **Programming Bitcoin** - Jimmy Song
Learn by building a Bitcoin library from scratch in Python.
- Website: https://programmingbitcoin.com/

#### **Grokking Bitcoin** - Kalle Rosenbaum
Visual, beginner-friendly approach to understanding Bitcoin.
- Website: https://www.manning.com/books/grokking-bitcoin

#### **Layered Money** - Nik Bhatia
Understanding monetary layers and Bitcoin's role in the system.

#### **The Blocksize War** - Jonathan Bier
History of Bitcoin's governance crisis and scaling debate.
- Free Online: https://blog.bitmex.com/the-blocksize-war/

---

### Foundational Papers

#### **Bitcoin: A Peer-to-Peer Electronic Cash System** (2008)
Satoshi Nakamoto's original Bitcoin whitepaper.
- PDF: https://bitcoin.org/bitcoin.pdf

#### **Hashcash - A Denial of Service Counter-Measure** (2002)
Adam Back's proof-of-work system that inspired Bitcoin mining.
- PDF: http://www.hashcash.org/papers/hashcash.pdf

#### **b-money** (1998)
Wei Dai's early cryptocurrency concept.
- Original: http://www.weidai.com/bmoney.txt

#### **Bit Gold** (2005)
Nick Szabo's Bitcoin precursor concept.
- Article: https://unenumerated.blogspot.com/2005/12/bit-gold.html

---

### Advanced Papers

#### **The Bitcoin Lightning Network** - Poon & Dryja
Original Lightning Network whitepaper.
- PDF: https://lightning.network/lightning-network-paper.pdf

#### **Schnorr Signatures for secp256k1**
Technical specification for Schnorr signatures in Bitcoin.
- BIP 340: https://github.com/bitcoin/bips/blob/master/bip-0340.mediawiki

#### **Taproot**
Technical specifications for Taproot upgrade (BIPs 340, 341, 342).
- BIP 341: https://github.com/bitcoin/bips/blob/master/bip-0341.mediawiki

---

## 🌐 Official Documentation

### Core Resources

- **Bitcoin.org** - https://bitcoin.org/
  - Official Bitcoin project website
  - Wallets, documentation, getting started guides

- **Bitcoin Core** - https://bitcoincore.org/
  - Reference implementation of Bitcoin
  - GitHub: https://github.com/bitcoin/bitcoin

- **Bitcoin Developer Documentation** - https://developer.bitcoin.org/
  - API references, guides, and examples
  - RPC documentation

- **Bitcoin Improvement Proposals (BIPs)** - https://github.com/bitcoin/bips
  - All Bitcoin protocol improvements
  - Searchable: https://bips.dev/

- **Bitcoin Optech** - https://bitcoinops.org/
  - Weekly technical newsletter (essential!)
  - Scaling resources and compatibility matrix
  - Workshop materials

---

## 📺 Video Resources

### Online Courses

#### **Bitcoin & Cryptocurrency Technologies** - Princeton University
Comprehensive free course with companion book.
- Coursera: https://www.coursera.org/learn/cryptocurrency
- Companion Book: https://bitcoinbook.cs.princeton.edu/

#### **Saylor Academy - Bitcoin for Everybody**
Beginner-friendly Bitcoin education.
- Course: https://learn.saylor.org/course/view.php?id=468

#### **Chaincode Labs Seminars**
Advanced technical seminars for developers.
- Website: https://learning.chaincode.com/

#### **Base58 Bitcoin Protocol Development**
Comprehensive Bitcoin Core development course.
- Website: https://www.base58.school/

---

### YouTube Channels

- **Andreas Antonopoulos** - https://www.youtube.com/@aantonop
  - Bitcoin education, Q&A, talks

- **Bitcoin Magazine** - https://www.youtube.com/@BitcoinMagazine
  - News, interviews, documentaries

- **BTC Sessions** - https://www.youtube.com/@BTCSessions
  - Practical tutorials and guides

- **Ministry of Nodes** - https://www.youtube.com/@MinistryofNodes
  - Node operation and setup tutorials

---

### Conference Archives

- **Bitcoin Conference** - https://www.youtube.com/c/BitcoinConference
- **Baltic Honeybadger** - https://www.youtube.com/@BalticHoneybadger
- **Adopting Bitcoin** - https://www.youtube.com/@adoptingbitcoin

---

## 🛠️ Development Resources

### Core Development

- **Bitcoin Core GitHub** - https://github.com/bitcoin/bitcoin
  - Main Bitcoin repository
  - Issues, pull requests, releases

- **Bitcoin Core Dev Meetings** - https://bitcoincore.org/en/meetings/
  - IRC logs and meeting transcripts

- **Bitcoin Core PR Review Club** - https://bitcoincore.reviews/
  - Learn by reviewing pull requests
  - Weekly meetings

- **Delving Bitcoin** - https://delvingbitcoin.org/
  - Technical discussion forum for Bitcoin development

---

### Learning to Build

- **Learn Me a Bitcoin** - https://learnmeabitcoin.com/
  - Visual technical guide with interactive tools
  - Build Tools: https://learnmeabitcoin.com/tools/

- **Bitcoin Dev Kit (BDK)** - https://bitcoindevkit.org/
  - Modern toolkit for building Bitcoin wallets
  - GitHub: https://github.com/bitcoindevkit/bdk

- **BTCPay Server** - https://btcpayserver.org/
  - Self-hosted Bitcoin payment processor
  - GitHub: https://github.com/btcpayserver/btcpayserver

- **LND (Lightning Network Daemon)** - https://github.com/lightningnetwork/lnd
  - Lightning Labs' Lightning implementation

- **Core Lightning (CLN)** - https://github.com/ElementsProject/lightning
  - Blockstream's Lightning implementation

---

### Testing Networks

- **Bitcoin Testnet Faucet** - https://testnet-faucet.mempool.co/
- **Bitcoin Testnet Explorer** - https://mempool.space/testnet
- **Signet Faucet** - https://signetfaucet.com/
- **Bitcoin Regtest** - Local testing environment (built into Bitcoin Core)

---

## 🔍 Explorers & Tools

### Block Explorers

- **Mempool.space** - https://mempool.space/
  - Best open-source explorer
  - Fee estimation, transaction tracking, Lightning explorer

- **Blockchair** - https://blockchair.com/bitcoin
  - Multi-chain explorer with advanced search

- **Blockchain.com Explorer** - https://www.blockchain.com/explorer

- **OXT** - https://oxt.me/
  - Advanced analytics and privacy analysis tools

---

### Analysis Tools

- **Clark Moody Bitcoin Dashboard** - https://bitcoin.clarkmoody.com/dashboard/
  - Real-time metrics and network statistics

- **Bitcoin Visuals** - https://bitcoinvisuals.com/
  - Charts and statistics about the Bitcoin network

- **Coin Metrics** - https://charts.coinmetrics.io/
  - Professional network data and analytics

- **Glassnode Studio** - https://studio.glassnode.com/
  - Advanced on-chain analytics (freemium)

---

### Fee & Mempool Tools

- **Mempool.space** - https://mempool.space/
  - Live mempool visualization
  - Fee estimation and transaction accelerator

- **Johoe's Bitcoin Mempool Statistics** - https://jochen-hoenicke.de/queue/
  - Historical mempool data and visualization

- **Bitcoin Fee Estimator** - https://bitcoiner.live/
  - Real-time fee recommendations

---

### Transaction & Script Tools

- **Bitcoin Transaction Size Calculator** - https://bitcoinops.org/en/tools/calc-size/
  - Calculate transaction sizes and fees

- **Transaction Decoder** - https://live.blockcypher.com/btc/decodetx/
  - Decode raw transactions

- **Bitcoin Script Playground** - https://bitcoin.sipa.be/miniscript/
  - Test and visualize Bitcoin scripts

- **Miniscript** - https://bitcoin.sipa.be/miniscript/
  - Compose Bitcoin scripts safely

---

## 📰 News & Updates

### Newsletters

- **Bitcoin Optech Newsletter** - https://bitcoinops.org/en/newsletters/
  - Weekly technical newsletter (essential for developers!)

- **Bitcoin Magazine** - https://bitcoinmagazine.com/
  - News, analysis, and opinion

- **The Bitcoin Times** - https://medium.com/the-bitcoin-times
  - Essays and long-form content

---

### Podcasts

- **What Bitcoin Did** - https://www.whatbitcoindid.com/
  - Interviews with influential Bitcoin figures

- **Bitcoin Audible** - https://bitcoinaudible.com/
  - Guy Swann reads and discusses Bitcoin articles

- **Stephan Livera Podcast** - https://stephanlivera.com/
  - Technical and economic discussions

- **Tales from the Crypt** - https://tftc.io/podcasts/
  - Bitcoin news and interviews

- **Chaincode Podcast** - https://podcast.chaincode.com/
  - Technical deep dives with Bitcoin developers

---

### Social & Discussion Platforms

- **Bitcoin Stack Exchange** - https://bitcoin.stackexchange.com/
  - Technical Q&A (highly recommended!)

- **Bitcoin Reddit** - https://www.reddit.com/r/Bitcoin/
  - General Bitcoin discussion

- **Bitcoin Talk Forum** - https://bitcointalk.org/
  - Original Bitcoin forum, historical discussions

- **Stacker News** - https://stacker.news/
  - Bitcoin-focused discussion with Lightning micropayments

- **Twitter/X** - Follow @bitcoin, @bitcoinops, @BitcoinMagazine
  - Real-time updates and community

---

## 🔐 Security & Privacy

### Key Management & Cold Storage

- **Glacier Protocol** - https://glacierprotocol.org/
  - High-security cold storage protocol

- **Seedsigner** - https://seedsigner.com/
  - DIY air-gapped hardware wallet
  - GitHub: https://github.com/SeedSigner/seedsigner

- **Specter Desktop** - https://specter.solutions/
  - Multisig wallet interface
  - GitHub: https://github.com/cryptoadvance/specter-desktop

---

### Privacy Tools & Techniques

- **Samourai Wallet** - https://samouraiwallet.com/
  - Privacy-focused mobile wallet with Whirlpool CoinJoin

- **Wasabi Wallet** - https://wasabiwallet.io/
  - Desktop wallet with built-in CoinJoin

- **JoinMarket** - https://github.com/JoinMarket-Org/joinmarket-clientserver
  - Decentralized CoinJoin implementation

- **Bitcoin Privacy Guide** - https://bitcoiner.guide/privacy/
  - Comprehensive privacy best practices

---

### Tor & Anonymity

- **Running Bitcoin Core with Tor** - https://bitcoin.org/en/tor
  - Official guide for Tor integration

- **Tor Project** - https://www.torproject.org/
  - Anonymous communication network

- **Whonix** - https://www.whonix.org/
  - Anonymous operating system

---

## ⚡ Lightning Network

### Core Resources

- **Lightning Network** - https://lightning.network/
  - Official Lightning Network website

- **Lightning Labs** - https://lightning.engineering/
  - Company behind LND implementation

- **ACINQ** - https://acinq.co/
  - Company behind Eclair implementation

- **Blockstream** - https://blockstream.com/lightning/
  - Company behind Core Lightning (CLN)

---

### Learning Resources

- **Lightning Network Whitepaper** - https://lightning.network/lightning-network-paper.pdf
  - Original technical paper

- **Mastering the Lightning Network** - https://github.com/lnbook/lnbook
  - Free book by Antonopoulos, Osuntokun, and Pickhardt

- **Lightning Network Training** - https://lightningnetwork.plus/
  - Guides for channel management and node operation

---

### Tools & Explorers

- **1ML Lightning Network Explorer** - https://1ml.com/
  - Node and channel explorer

- **Amboss** - https://amboss.space/
  - Node analytics, tools, and explorer

- **Ride The Lightning (RTL)** - https://github.com/Ride-The-Lightning/RTL
  - Web UI for managing Lightning nodes

- **ThunderHub** - https://thunderhub.io/
  - Lightning node management interface

- **Terminal Web** - https://terminal.lightning.engineering/
  - LND cloud management

---

## 📚 Community Resources

### Comprehensive Guides

- **Bitcoiner.Guide** - https://bitcoiner.guide/
  - Guides for all levels and use cases

- **Bitcoin-Only** - https://bitcoin-only.com/
  - Curated list of Bitcoin-only resources

- **SatoABC** - https://www.satoabc.com/
  - Plain-English Bitcoin guides for beginners, including first purchase size, buying flow, wallet basics, fees, records, and safety checks before increasing exposure

- **Ministry of Nodes** - https://www.ministryofnodes.com.au/
  - Node guides and hardware recommendations

---

### Node Distributions

- **RaspiBlitz** - https://github.com/rootzoll/raspiblitz
  - Raspberry Pi Bitcoin/Lightning node

- **Umbrel** - https://umbrel.com/
  - User-friendly node OS with app store

- **MyNode** - https://mynodebtc.com/
  - Premium node solution

- **Start9** - https://start9.com/
  - Sovereign computing platform with Bitcoin node

---

### Developer Communities

- **Bitcoin Core Dev IRC** - #bitcoin-core-dev on Libera.Chat
- **Lightning Dev Mailing List** - https://lists.linuxfoundation.org/mailman/listinfo/lightning-dev
- **Bitcoin Dev Mailing List** - https://lists.linuxfoundation.org/mailman/listinfo/bitcoin-dev

---

### Educational Organizations

- **Chaincode Labs** - https://chaincode.com/
  - Bitcoin development residency program
  - Open-source funding

- **Brink** - https://brink.dev/
  - Bitcoin developer funding and mentorship

- **Spiral (formerly Square Crypto)** - https://spiral.xyz/
  - Open-source Bitcoin development

- **MIT Digital Currency Initiative** - https://dci.mit.edu/
  - Academic Bitcoin research

---

## 🎓 Academic Resources

### Research Paper Collections

- **Bitcoin Research** - https://docs.google.com/spreadsheets/d/1VaWhbAj7hWNdiE73P-W-wrl5a0WNgzjofmZXe0Rh5sg/
  - Comprehensive collection of Bitcoin research papers

- **Scaling Bitcoin** - https://scalingbitcoin.org/
  - Conference proceedings and research

---

### Universities Teaching Bitcoin

- **MIT** - Bitcoin and Blockchain courses
- **Stanford** - Bitcoin Engineering
- **UC Berkeley** - Blockchain at Berkeley
- **Princeton** - Bitcoin & Cryptocurrency Technologies

---

## 🔧 Advanced Topics

### Protocol & Consensus

- **Bitcoin Wiki - Protocol Rules** - https://en.bitcoin.it/wiki/Protocol_rules
  - Comprehensive protocol documentation

- **Bitcoin Improvement Proposals** - https://github.com/bitcoin/bips
  - All protocol improvement proposals

- **Optech Schnorr/Taproot Workshop** - https://bitcoinops.org/en/schorr-taproot-workshop/
  - Technical workshop materials

---

### Script & Smart Contracts

- **Miniscript** - https://bitcoin.sipa.be/miniscript/
  - Structured Bitcoin scripts

- **Bitcoin Script Wiki** - https://en.bitcoin.it/wiki/Script
  - Script operations reference

---

### Mining

- **Braiins** - https://braiins.com/
  - Mining pool and firmware

- **Stratum V2** - https://stratumprotocol.org/
  - Next-generation mining protocol

- **Bitcoin Mining Council** - https://bitcoinminingcouncil.com/
  - Mining industry coordination

---

## 📱 Wallets

### Mobile Wallets

- **Blue Wallet** - https://bluewallet.io/
  - Bitcoin and Lightning, user-friendly

- **Phoenix Wallet** - https://phoenix.acinq.co/
  - Lightning-focused, excellent UX

- **Breez** - https://breez.technology/
  - Non-custodial Lightning node in your pocket

- **Muun** - https://muun.com/
  - Bitcoin and Lightning, submarine swaps

---

### Desktop Wallets

- **Sparrow Wallet** - https://sparrowwallet.com/
  - Advanced features, privacy-focused, excellent UI

- **Electrum** - https://electrum.org/
  - Lightweight, mature, highly configurable

- **Specter Desktop** - https://specter.solutions/
  - Hardware wallet interface, multisig focus

---

### Hardware Wallets

- **Coldcard** - https://coldcard.com/
  - Bitcoin-only, air-gapped, advanced security

- **BitBox02** - https://shiftcrypto.ch/bitbox02/
  - Swiss-made, Bitcoin-only version available

- **SeedSigner** - https://seedsigner.com/
  - DIY, completely open-source, air-gapped

- **Trezor** - https://trezor.io/
  - Original hardware wallet, open-source

- **Ledger** - https://www.ledger.com/
  - Popular option, wide coin support

---

## 🌍 Bitcoin Organizations

### Development Organizations

- **Bitcoin Core** - https://bitcoincore.org/
- **MIT DCI** - https://dci.mit.edu/
- **Chaincode Labs** - https://chaincode.com/
- **Blockstream** - https://blockstream.com/
- **Lightning Labs** - https://lightning.engineering/

---

### Advocacy & Policy

- **Coin Center** - https://www.coincenter.org/
  - Policy advocacy and research

- **Human Rights Foundation Bitcoin Development Fund** - https://hrf.org/programs_posts/devfund/
  - Funding Bitcoin development for human rights

---

### Media Organizations

- **Bitcoin Magazine** - https://bitcoinmagazine.com/
  - Longest-running Bitcoin publication

- **CoinDesk** - https://www.coindesk.com/
  - Crypto news and analysis

- **The Block** - https://www.theblock.co/
  - Research and news

---

## 📊 Data & Statistics

### Market Data

- **CoinGecko** - https://www.coingecko.com/en/coins/bitcoin
  - Price data, market cap, trading volume

- **CoinMarketCap** - https://coinmarketcap.com/currencies/bitcoin/
  - Market statistics and data

---

### On-Chain Analytics

- **Blockchain.com Charts** - https://www.blockchain.com/charts
  - Historical blockchain data

- **BitInfoCharts** - https://bitinfocharts.com/bitcoin/
  - Network statistics and charts

- **Coin Metrics** - https://coinmetrics.io/
  - Professional-grade analytics

- **Glassnode** - https://glassnode.com/
  - Advanced on-chain metrics

---

### Network Health

- **Bitnodes** - https://bitnodes.io/
  - Map of reachable Bitcoin nodes

- **Bitcoin Node Map** - https://bitcoinmap.org/
  - Geographic distribution of nodes

- **Luke Dashjr's Node Count** - http://luke.dashjr.org/programs/bitcoin/files/charts/software.html
  - Historical node statistics

---

## 🤝 Contributing

Contributions are welcome! If you know of a valuable Bitcoin resource that's missing from this list, please:

1. Fork this repository
2. Add the resource with a brief description
3. Submit a pull request

**Quality over quantity** - we prioritize well-maintained, accurate, and valuable resources.

---

## 📄 License

This document is released into the public domain under [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/).

---

## 🙏 Acknowledgments

This resource list has been compiled from the Bitcoin community's collective knowledge. Special thanks to all the educators, developers, and contributors who make Bitcoin learning accessible.

---

**Last Updated:** November 2025

**Maintained by:** [mcps976](https://github.com/mcps976)
